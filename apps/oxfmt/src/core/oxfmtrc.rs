...
/// A single override entry compatible with Prettier's overrides format.
#[derive(Debug, Default, Clone, Deserialize, Serialize, JsonSchema)]
...
pub struct OxfmtOverrideConfig {
    ...
    ///
    /// Examples:
    /// - `["*.test.js"]` - matches all test files
    /// - `["*.md", "*.html"]` - matches markdown and HTML files
    /// - `["src/**/*.ts"]` - matches TypeScript files in src directory
    ///
    /// All patterns are relative to the Oxfmt configuration file.
    pub files: Vec<String>,
    /// Glob patterns to exclude from this override.
    /// Files matching these patterns will not have the override applied.
    #[serde(skip_serializing_if = "Option::is_none")]
